...
def solve_preissmann_newton(H_n, Q_n, dt, dx, B, n, S0, g, z_bed, theta, Q_bc, H_bc, gate_idx, gate_open, Cd):
    ...
    for k_iter in range(15):
        ...
        if y_down_k>0.9*gate_open:
            ...
            d_Qgate_dHup=Cd*B*gate_open*np.sqrt(g/2)/np.sqrt(delta_H_k) if delta_H_k>epsilon else 0
            M[j,2*i+2]=1; M[j,2*i+1]=-d_Qgate_dHup; M[j,2*i+3]=d_Qgate_dHup
