...
# --- 3. Initial Conditions (Corrected for Gate) ---
def calculate_initial_conditions_corrected(Q_val, H_down, z_bed_arr, B_val, n_val, S0_val, nx_val, dx_val, gate_idx, gate_open, cd_val):
    ...
    if y_down_gate > 0.9 * gate_open: # Submerged
        H[gate_idx] = (Q_val/(cd_val*gate_open*B_val))**2/(2*g) + H[gate_idx+1]
    else: # Free flow
        y_up_gate = (Q_val/(cd_val*gate_open*B_val))**2/(2*g)
        H[gate_idx] = y_up_gate + z_bed_arr[gate_idx]
    # Upstream part
...
# --- 4. Preissmann Solver with Newton-Raphson Iteration ---
def solve_preissmann_newton(H_n, Q_n, dt, dx, B, n, S0, g, z_bed, theta, Q_bc, H_bc, gate_idx, gate_open, Cd):
    nx = len(H_n)
    H_k, Q_k = H_n.copy(), Q_n.copy() # k is iteration counter

    for k_iter in range(15): # Newton-Raphson loop
        M = np.zeros((2*nx, 2*nx)); P = np.zeros(2*nx)
        epsilon = 1e-5

        for i in range(nx - 1):
            if i == gate_idx: continue
            # --- Coefficients for Jacobian Matrix M and function vector P ---
            y_i_k, y_i1_k = H_k[i]-z_bed[i], H_k[i+1]-z_bed[i+1]
            A_i_k, A_i1_k = B*y_i_k, B*y_i1_k
            R_i_k, R_i1_k = A_i_k/(B+2*y_i_k), A_i1_k/(B+2*y_i1_k)

            # --- Continuity Equation F1 ---
            j = 2*i
            M[j, 2*i], M[j, 2*i+2] = -theta/dx, theta/dx
            M[j, 2*i+1], M[j, 2*i+3] = B/(2*dt), B/(2*dt)
            P[j] = theta/dx*(Q_k[i+1]-Q_k[i]) + (1-theta)/dx*(Q_n[i+1]-Q_n[i]) + \
                   B/(2*dt)*( (H_k[i]-H_n[i]) + (H_k[i+1]-H_n[i+1]) )

            # --- Momentum Equation F2 ---
            j = 2*i+1
            Q_avg_k, A_avg_k, R_avg_k = (Q_k[i]+Q_k[i+1])/2, (A_i_k+A_i1_k)/2, (R_i_k+R_i1_k)/2
            Sf_k = n**2 * Q_avg_k * abs(Q_avg_k) / (A_avg_k**2 * R_avg_k**(4/3)) if A_avg_k > epsilon else 0

            # Partial derivatives for Jacobian M
            M[j, 2*i]   = 1/(2*dt) + theta*2*Q_k[i]/(A_i_k*dx)
            M[j, 2*i+1] = theta*(-g*A_i_k/dx - (Q_k[i]**2*B/(A_i_k**2*dx)))
            M[j, 2*i+2] = 1/(2*dt) - theta*2*Q_k[i+1]/(A_i1_k*dx)
            M[j, 2*i+3] = theta*(g*A_i1_k/dx + (Q_k[i+1]**2*B/(A_i1_k**2*dx)))

            # Function vector P
            Sf_n = n**2 * (Q_n[i]+Q_n[i+1])/2 * abs((Q_n[i]+Q_n[i+1])/2) / (((A_i_k+A_i1_k)/2)**2 * ((R_i_k+R_i1_k)/2)**(4/3)) if A_avg_k > epsilon else 0
            P[j] = (Q_avg_k - (Q_n[i]+Q_n[i+1])/2)/dt + \
                   theta/dx*((Q_k[i+1]**2/A_i1_k) - (Q_k[i]**2/A_i_k)) + (1-theta)/dx*((Q_n[i+1]**2/A_i1_k) - (Q_n[i]**2/A_i_k)) + \
                   g*theta*((A_i_k+A_i1_k)/2)/dx*(H_k[i+1]-H_k[i]) + g*(1-theta)*((A_i_k+A_i1_k)/2)/dx*(H_n[i+1]-H_n[i]) - \
                   g*theta*A_avg_k*(S0-Sf_k) - g*(1-theta)*A_avg_k*(S0-Sf_n)
        ...
        if y_down_k > 0.9*gate_open:
            Q_gate_k = Cd*B*gate_open*np.sqrt(2*g*max(0,H_k[i]-H_k[i+1])); M[j,j+2]=1; P[j] = Q_k[i+1]-Q_gate_k
        else:
            Q_gate_k = Cd*B*gate_open*np.sqrt(2*g*max(0,y_up_k)); M[j,j+2]=1; P[j] = Q_k[i+1]-Q_gate_k
    ...
    print(f"Warning: Newton-Raphson did not converge after {k_iter+1} iterations.")
    return H_k, Q_k

# --- 5. Main Loop ---
nt = int(T_total / dt)
H_results, Q_results = [], []
print(f"Using final, correct Preissmann scheme with Newton-Raphson. dt={dt}s, nt={nt} steps.")
...
print("Generating final Preissmann GIF animation...")
